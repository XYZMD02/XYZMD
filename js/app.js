document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 安全超时：如果初始化卡住超过 12 秒，强制进入页面
    const forceEnterTimeout = setTimeout(() => {
        console.warn('[boot] 初始化超时（12s），强制进入页面');
        updateLoader('加载超时，已强制进入...', '100%');
        hideWelcomeScreen();
    }, 12000);
    ...
    try {
        ...
        clearTimeout(forceEnterTimeout);
