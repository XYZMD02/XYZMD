...
window.openMoyuModal = function () {
    ...
    if (typeof window.renderMoodLibrary === 'function') window.renderMoodLibrary();
...
// ==================== 心情库管理 ====================
window.renderMoodLibrary = function () {
    const container = document.getElementById('mood-library-list');
    if (!container) return;
    const moods = window.moodLibrary || [];
    let listHtml = '';
    if (moods.length === 0) {
        listHtml = '<div style="text-align:center;padding:20px;color:var(--text-secondary);">还没有心情词汇</div>';
    } else {
        listHtml = moods.map(function(item, index) {
            return '<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:var(--secondary-bg);border-radius:8px;margin-bottom:6px;border:1px solid var(--border-color);">' +
                '<span>' + window.escapeHtml(item) + '</span>' +
                '<button onclick="window.removeMoodItem(' + index + ')" style="background:none;border:none;color:#ff6b6b;cursor:pointer;">✕</button>' +
            '</div>';
        }).join('');
    }
    container.innerHTML = listHtml +
        '<div style="display:flex;gap:8px;margin-top:10px;margin-bottom:10px;">' +
            '<input type="text" id="mood-new-input" placeholder="输入心情词汇..." style="flex:1;padding:8px;border:1px solid var(--border-color);border-radius:8px;background:var(--primary-bg);color:var(--text-primary);font-size:13px;font-family:var(--font-family);outline:none;">' +
            '<button onclick="window.addMoodItem()" style="padding:8px 16px;background:var(--accent-color);color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:13px;font-family:var(--font-family);">添加</button>' +
        '</div>';
};

window.addMoodItem = function () {
    var input = document.getElementById('mood-new-input');
    if (!input) return;
    var text = input.value.trim();
    if (!text) return;
    if (!window.moodLibrary) window.moodLibrary = [];
    window.moodLibrary.push(text);
    console.log('[addMoodItem] 当前 moodLibrary:', window.moodLibrary);
    if (typeof saveData === 'function') saveData();
    input.value = '';
    window.renderMoodLibrary();
};

window.removeMoodItem = function (index) {
    if (!window.moodLibrary || index < 0 || index >= window.moodLibrary.length) return;
    window.moodLibrary.splice(index, 1);
    window.renderMoodLibrary();
    if (typeof throttledSaveData === 'function') throttledSaveData();
};
...
window.importMoodToCheckin = function() {
    const moodQuestions = window.moodLibrary || [];   // ← 从心情库读取
    if (!window.checkinQuestions) window.checkinQuestions = [];
    let added = 0;
    ...
    window.renderCheckinQuestionList();
    if (typeof throttledSaveData === 'function') throttledSaveData();
