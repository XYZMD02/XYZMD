...
window.renderMoodLibrary = function () {
    const listEl = document.getElementById('mood-library-list');
    if (!listEl) return;
    const moods = window.moodLibrary || [];
    if (moods.length === 0) {
        listEl.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-secondary);">还没有心情词汇</div>';
        return;
    }
    listEl.innerHTML = moods.map((item, index) => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:var(--secondary-bg);border-radius:8px;margin-bottom:6px;border:1px solid var(--border-color);">
...
window.addMoodItem = function () {
    const input = document.getElementById('mood-new-input');
    if (!input) return;
