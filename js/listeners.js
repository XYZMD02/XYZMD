...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    setupAvatarFrameSettings();
            ...
            const _extraToggleSync = {
                ...
                '#moyu-debug-mode-toggle': 'moyuDebugMode',
                '#moyu-show-detail-toggle': 'moyuShowDetail',
                '#checkin-partner-active-toggle': 'checkinPartnerActive',
                '#checkin-notify-toggle': 'checkinNotify',
                '#chat-checkin-active-toggle': 'checkinPartnerActive',
...
// 对方撤回概率
const withdrawChanceSlider = document.getElementById('withdraw-chance-slider');
const withdrawChanceDisplay = document.getElementById('withdraw-chance-display');
if (withdrawChanceSlider) {
    withdrawChanceSlider.value = settings.partnerWithdrawChance || 0;
    if (withdrawChanceDisplay) withdrawChanceDisplay.textContent = (settings.partnerWithdrawChance || 0) + '%';
    withdrawChanceSlider.addEventListener('input', function() {
        settings.partnerWithdrawChance = parseInt(this.value);
        if (withdrawChanceDisplay) withdrawChanceDisplay.textContent = this.value + '%';
    });
    withdrawChanceSlider.addEventListener('change', function() {
        throttledSaveData();
    });
}
...
// 摸鱼调试模式设置
const moyuDebugModeToggle = document.getElementById('moyu-debug-mode-toggle');
const moyuDebugSettings = document.getElementById('moyu-debug-settings');
const moyuReportMinInterval = document.getElementById('moyu-report-min-interval');
const moyuReportMinVal = document.getElementById('moyu-report-min-val');
const moyuReportMaxInterval = document.getElementById('moyu-report-max-interval');
const moyuReportMaxVal = document.getElementById('moyu-report-max-val');
const moyuReportMinUnit = document.getElementById('moyu-report-min-unit');
const moyuReportMaxUnit = document.getElementById('moyu-report-max-unit');

const updateMoyuDebugModeUI = () => {
    if (moyuDebugModeToggle) moyuDebugModeToggle.classList.toggle('active', !!settings.moyuDebugMode);
    if (moyuDebugSettings) moyuDebugSettings.style.display = settings.moyuDebugMode ? 'block' : 'none';
    if (moyuReportMinInterval) moyuReportMinInterval.value = settings.moyuReportMinInterval || 10;
    var minUnit = settings.moyuReportMinUnit || 'minutes';
    if (moyuReportMinVal) moyuReportMinVal.textContent = (settings.moyuReportMinInterval || 10) + (minUnit === 'hours' ? '小时' : '分钟');
    if (moyuReportMinUnit) moyuReportMinUnit.value = minUnit;
    if (moyuReportMaxInterval) moyuReportMaxInterval.value = settings.moyuReportMaxInterval || 30;
    var maxUnit = settings.moyuReportMaxUnit || 'minutes';
    if (moyuReportMaxVal) moyuReportMaxVal.textContent = (settings.moyuReportMaxInterval || 30) + (maxUnit === 'hours' ? '小时' : '分钟');
    if (moyuReportMaxUnit) moyuReportMaxUnit.value = maxUnit;
};

updateMoyuDebugModeUI();

if (moyuDebugModeToggle) {
    moyuDebugModeToggle.addEventListener('click', () => {
        settings.moyuDebugMode = !settings.moyuDebugMode;
        updateMoyuDebugModeUI();
        throttledSaveData();
        showNotification(`手动调试模式已${settings.moyuDebugMode ? '开启' : '关闭'}`, 'success');
    });
}

if (moyuReportMinInterval) {
    moyuReportMinInterval.addEventListener('input', (e) => {
        const val = parseInt(e.target.value);
        settings.moyuReportMinInterval = val;
        var unitEl = document.getElementById('moyu-report-min-unit');
        var unit = unitEl ? unitEl.value : 'minutes';
        if (moyuReportMinVal) moyuReportMinVal.textContent = val + (unit === 'hours' ? '小时' : '分钟');
    });
    moyuReportMinInterval.addEventListener('change', () => {
        throttledSaveData();
        if (typeof manageMoyuAutoGenerateTimer === 'function') manageMoyuAutoGenerateTimer();
    });
}

if (moyuReportMaxInterval) {
    moyuReportMaxInterval.addEventListener('input', (e) => {
        const val = parseInt(e.target.value);
        settings.moyuReportMaxInterval = val;
        var unitEl = document.getElementById('moyu-report-max-unit');
        var unit = unitEl ? unitEl.value : 'minutes';
        if (moyuReportMaxVal) moyuReportMaxVal.textContent = val + (unit === 'hours' ? '小时' : '分钟');
    });
    moyuReportMaxInterval.addEventListener('change', () => {
        throttledSaveData();
        if (typeof manageMoyuAutoGenerateTimer === 'function') manageMoyuAutoGenerateTimer();
    });
}
...
// 聊天设置 - 查岗开关
const chatCheckinActiveToggle = document.getElementById('chat-checkin-active-toggle');
const chatCheckinSettings = document.getElementById('chat-checkin-settings');

const updateChatCheckinUI = () => {
    if (chatCheckinActiveToggle) chatCheckinActiveToggle.classList.toggle('active', !!settings.checkinPartnerActive);
    if (chatCheckinSettings) chatCheckinSettings.style.display = settings.checkinPartnerActive ? 'block' : 'none';
    // 同步间隔滑块和单位
    var minSlider = document.getElementById('checkin-min-interval-slider');
    var maxSlider = document.getElementById('checkin-max-interval-slider');
    var minUnit = document.getElementById('checkin-min-unit');
    var maxUnit = document.getElementById('checkin-max-unit');
    var minDisplay = document.getElementById('checkin-min-val-display');
    var maxDisplay = document.getElementById('checkin-max-val-display');
    if (minSlider) minSlider.value = settings.checkinMinInterval || 10;
    if (maxSlider) maxSlider.value = settings.checkinMaxInterval || 30;
    var mUnit = settings.checkinMinUnit || 'minutes';
    var xUnit = settings.checkinMaxUnit || 'minutes';
    if (minUnit) minUnit.value = mUnit;
    if (maxUnit) maxUnit.value = xUnit;
    if (minDisplay) minDisplay.textContent = (settings.checkinMinInterval || 10) + (mUnit === 'hours' ? '小时' : '分钟');
    if (maxDisplay) maxDisplay.textContent = (settings.checkinMaxInterval || 30) + (xUnit === 'hours' ? '小时' : '分钟');
};

updateChatCheckinUI();

if (chatCheckinActiveToggle) {
    chatCheckinActiveToggle.addEventListener('click', () => {
        settings.checkinPartnerActive = !settings.checkinPartnerActive;
        updateChatCheckinUI();
        // 同步查岗设置弹窗中的开关
        const checkinPartnerToggle = document.getElementById('checkin-partner-active-toggle');
        if (checkinPartnerToggle) checkinPartnerToggle.classList.toggle('active', !!settings.checkinPartnerActive);
        if (typeof throttledSaveData === 'function') throttledSaveData();
        if (typeof showNotification === 'function') showNotification(`对方查岗已${settings.checkinPartnerActive ? '开启' : '关闭'}`, 'success');
        if (typeof scheduleNextCheckin === 'function') scheduleNextCheckin();
    });
}

// 摸鱼汇报间隔单位
['moyu-report-min-unit', 'moyu-report-max-unit'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.addEventListener('change', function() {
        settings[id.replace(/-/g, '_')] = this.value;
        // 更新显示值
        var sliderId = id.replace('-unit', '');
        var slider = document.getElementById(sliderId);
        var display = document.getElementById(sliderId.replace('-interval', '-val'));
        if (slider && display) {
            var val = slider.value;
            display.textContent = val + (this.value === 'hours' ? '小时' : '分钟');
        }
        throttledSaveData();
        if (typeof manageMoyuAutoGenerateTimer === 'function') manageMoyuAutoGenerateTimer();
    });
...
// 查岗间隔
['checkin-min-interval-slider', 'checkin-max-interval-slider'].forEach(function(id) {
    var el = document.getElementById(id);
    if (!el) return;
    el.addEventListener('input', function() {
        var isMin = id.includes('-min-');
        var unitId = isMin ? 'checkin-min-unit' : 'checkin-max-unit';
        var unitEl = document.getElementById(unitId);
        var unit = unitEl ? unitEl.value : 'minutes';
        var displayId = isMin ? 'checkin-min-val-display' : 'checkin-max-val-display';
        var display = document.getElementById(displayId);
        if (display) display.textContent = this.value + (unit === 'hours' ? '小时' : '分钟');
        var settingKey = isMin ? 'checkinMinInterval' : 'checkinMaxInterval';
...
['checkin-min-unit', 'checkin-max-unit'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.addEventListener('change', function() {
        var isMin = id.includes('-min');
        var settingKey = isMin ? 'checkinMinUnit' : 'checkinMaxUnit';
        settings[settingKey] = this.value;
        var sliderId = isMin ? 'checkin-min-interval-slider' : 'checkin-max-interval-slider';
        var slider = document.getElementById(sliderId);
        var displayId = isMin ? 'checkin-min-val-display' : 'checkin-max-val-display';
        var display = document.getElementById(displayId);
        if (slider && display) display.textContent = slider.value + (this.value === 'hours' ? '小时' : '分钟');
        throttledSaveData();
        if (typeof scheduleNextCheckin === 'function') scheduleNextCheckin();
    });
});
...
if (sessionId === currentSessionId) {
    ...
    showNotification('会话已删除', 'success');
        ...
        function initCoreListeners() {
            ...
            DOMElements.continueBtn.addEventListener('click', function(e) {
