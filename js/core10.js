...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'checkinQuestions',       val: () => localforage.setItem(getStorageKey('checkinQuestions'), window.checkinQuestions || []) },
        { key: 'checkinRecords',         val: () => localforage.setItem(getStorageKey('checkinRecords'), window.checkinRecords || []) },
        { key: 'phoneStatusLibrary',      val: () => localforage.setItem(getStorageKey('phoneStatusLibrary'), window.phoneStatusLibrary || []) },
        { key: 'phoneStatusGroups',       val: () => localforage.setItem(getStorageKey('phoneStatusGroups'), window.phoneStatusGroups || []) },
...
function manageAutoSendTimer() {
    ...
    if (window.sessionList && window.SESSION_ID) {
        const session = window.sessionList.find(s => s.id === window.SESSION_ID);
        if (session && !session.paused && session.messages && session.messages.length > 0) {
            const randomDelay = settings.autoSendDelayMin + Math.random() * (settings.autoSendDelayMax - settings.autoSendDelayMin);
            const _timerSessionId = window.SESSION_ID;
            window._pendingReplyTimer = setTimeout(() => {
                // 会话隔离：如果已切换到其他会话，不触发 simulateReply
                if (window.SESSION_ID !== _timerSessionId) return;
                simulateReply();
            }, randomDelay);
        }
    }
...
// ===== 聊天界面独立的点歌申请定时器 =====
// 从悬浮播放器歌单随机选歌，向用户发送一起听申请（与一起听的推荐独立）
let chatSongRecTimer = null;
function manageChatSongRecTimer() {
    if (chatSongRecTimer) {
        clearInterval(chatSongRecTimer);
        chatSongRecTimer = null;
    }
    // 只在悬浮播放器有歌曲且陪伴模式开启时才启动
    if (typeof window.MusicBuddyApp !== 'object' || !window.MusicBuddyApp.getRandomSongFromPlaylists) return;
    // 随机间隔 120~300 秒
    var intervalMs = (120 + Math.random() * 180) * 1000;
    chatSongRecTimer = setInterval(function() {
        // 检查是否在聊天界面（不在首页或一起听页面）
        var homeContainer = document.getElementById('home-container');
        var mbPage = document.getElementById('music-buddy-page');
        if (homeContainer && homeContainer.style.display === 'none') return; // 不在主页/聊天
        if (mbPage && mbPage.style.display !== 'none') return; // 在一起听页面，不弹
        if (document.getElementById('chat-song-rec-modal')) return; // 已有弹窗
        // 从悬浮播放器歌单随机取歌
        var song = window.MusicBuddyApp.getRandomSongFromPlaylists();
        if (!song) return;
        // 随机延迟 1~3 秒后弹出，显得自然
        var partnerName = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '对方';
        setTimeout(function() {
            if (document.getElementById('chat-song-rec-modal')) return;
            if (typeof window.MusicBuddyApp.recommendSongToChat === 'function') {
                var pAvatar = '';
                try { if(typeof settings !== 'undefined' && settings.partnerAvatar) pAvatar = settings.partnerAvatar; } catch(e){}
                window.MusicBuddyApp.recommendSongToChat(song, partnerName, pAvatar);
            }
...
function manageMoyuAutoGenerateTimer() {
    ...
    // 没有进行中的会话，使用用户设置的间隔开始新会话
    const minUnit = settings.moyuReportMinUnit || 'minutes';
    const maxUnit = settings.moyuReportMaxUnit || 'minutes';
    const minVal = Math.max(1, parseInt(settings.moyuReportMinInterval) || 10);
    const maxVal = Math.max(minVal, parseInt(settings.moyuReportMaxInterval) || 30);
    const minMs = minUnit === 'hours' ? minVal * 60 * 60 * 1000 : minVal * 60 * 1000;
    const maxMs = maxUnit === 'hours' ? maxVal * 60 * 60 * 1000 : maxVal * 60 * 1000;
    const nextSessionDelay = Math.floor(Math.random() * (maxMs - minMs + 1)) + minMs;
...
function scheduleNextMoyuMessage() {
    ...
    if (now >= moyuWorkSession.endTime) {
        finishMoyuWorkSession();
        // 会话结束后，使用用户设置的间隔开始新会话
        const sMinUnit = settings.moyuReportMinUnit || 'minutes';
        const sMaxUnit = settings.moyuReportMaxUnit || 'minutes';
        const sMinVal = Math.max(1, parseInt(settings.moyuReportMinInterval) || 10);
        const sMaxVal = Math.max(sMinVal, parseInt(settings.moyuReportMaxInterval) || 30);
        const sMinMs = sMinUnit === 'hours' ? sMinVal * 60 * 60 * 1000 : sMinVal * 60 * 1000;
        const sMaxMs = sMaxUnit === 'hours' ? sMaxVal * 60 * 60 * 1000 : sMaxVal * 60 * 1000;
        const sNextDelay = Math.floor(Math.random() * (sMaxMs - sMinMs + 1)) + sMinMs;
        moyuSessionTimer = setTimeout(() => {
            generateRandomMoyuRecord();
        }, sNextDelay);
        return;
    ...
    const minMs = minUnit === 'hours' ? minVal * 60 * 60 * 1000 : minVal * 60 * 1000;
    const maxMs = maxUnit === 'hours' ? maxVal * 60 * 60 * 1000 : maxVal * 60 * 1000;
    const messageInterval = Math.floor(Math.random() * (maxMs - minMs + 1)) + minMs;
    // 确保不会超出会话结束时间
...
function generateRandomMoyuRecord() {
    ...
    if (activities.length === 0) {
        if (typeof showNotification === 'function') {
            showNotification('暂无摸鱼字卡，请到自定义回复库 → 摸鱼管理 → 摸鱼活动中添加', 'warning');
        }
        return;
    }
...
function finishMoyuWorkSession() {
    ...
    // 如果功能仍开启，安排下一次工作会话（使用用户设置的间隔）
    if (settings.moyuAutoGenerateEnabled) {
        const fMinUnit = settings.moyuReportMinUnit || 'minutes';
        const fMaxUnit = settings.moyuReportMaxUnit || 'minutes';
        const fMinVal = Math.max(1, parseInt(settings.moyuReportMinInterval) || 10);
        const fMaxVal = Math.max(fMinVal, parseInt(settings.moyuReportMaxInterval) || 30);
        const fMinMs = fMinUnit === 'hours' ? fMinVal * 60 * 60 * 1000 : fMinVal * 60 * 1000;
        const fMaxMs = fMaxUnit === 'hours' ? fMaxVal * 60 * 60 * 1000 : fMaxVal * 60 * 1000;
        const fNextDelay = Math.floor(Math.random() * (fMaxMs - fMinMs + 1)) + fMinMs;
        moyuSessionTimer = setTimeout(() => {
            generateRandomMoyuRecord();
        }, fNextDelay);
    }
...
if (!isBatchMode && type === 'normal') {
    ...
    window._pendingReplyTimer = null;
        ...
        window.simulateReply = function() {
            // 会话隔离：捕获当前回复批次代次和会话ID
            const _replyGen = window._replyGeneration || 0;
            const _sessionId = window.SESSION_ID;

            function showTypingIndicator() {
                if (!settings.typingIndicatorEnabled) return;
                // 只在当前会话显示正在输入
                if (window.SESSION_ID !== _sessionId) return;
                const tiWrapper = document.getElementById('typing-indicator-wrapper');
                const tiLabel = document.getElementById('typing-indicator-label');
                const tiAvatar = document.getElementById('typing-indicator-avatar');
                if (tiLabel) tiLabel.textContent = (settings.partnerName || '对方') + ' 正在输入';
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            // 不在外部同步调用，移到第一个 setTimeout 内部，避免切换会话后旧会话 typing 残留
            let delay = 0;
            ...
            for (let i = 0; i < replyCount; i++) {
                ...
                setTimeout(() => {
                    try {
                    // 会话隔离：如果代次已变或会话已切换，丢弃此次回复
                    if ((window._replyGeneration || 0) !== _replyGen) return;
                    if (window.SESSION_ID !== _sessionId) return;
                    // 第一条回复时才显示正在输入（在代次/会话检查之后）
                    ...
                    // 撤回逻辑
                    if (settings.partnerWithdrawChance > 0 && Math.random() * 100 < settings.partnerWithdrawChance) {
                        const withdrawDelay = 2000 + Math.random() * 5000;
                        setTimeout(function() {
                            const msgEl = document.querySelector('[data-msg-id="' + newMsgId + '"]');
                            if (msgEl) {
                                msgEl.style.transition = 'opacity 0.3s, max-height 0.3s, margin 0.3s, padding 0.3s';
                                ...
                                setTimeout(function() {
                                    msgEl.remove();
                                    addMessage({ id: Date.now(), text: (settings.partnerName || '对方') + ' 撤回了一条消息', timestamp: new Date(), type: 'system' });
                                    throttledSaveData();
                                }, 350);
                            }
                        }, withdrawDelay);
            ...
            overlay.innerHTML = `
                <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:88%;max-width:360px;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:9px;margin-bottom:20px;">
                        ...
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px 12px;border:1px solid var(--border-color);border-radius:12px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
    <input type="checkbox" id="_exp_checkin" checked style="accent-color:var(--accent-color);width:15px;height:15px;">
    <i class="fas fa-question-circle" style="color:var(--accent-color);width:16px;text-align:center;"></i>
    <span>查岗问题库</span>
</label>
<label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px 12px;border:1px solid var(--border-color);border-radius:12px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
    <input type="checkbox" id="_exp_mood" checked style="accent-color:var(--accent-color);width:15px;height:15px;">
    <i class="fas fa-smile" style="color:var(--accent-color);width:16px;text-align:center;"></i>
    <span>心情库</span>
</label>
                    </div>
                    <div style="display:flex;gap:10px;">
            ...
            if (_expConfirmBtn) _expConfirmBtn.onclick = function() {
                ...
                const inclThemes   = !!document.getElementById('_exp_themes')?.checked;
                const inclCheckin = !!document.getElementById('_exp_checkin')?.checked;
                ...
                try {
                    ...
                    if (inclCheckin) {
                        exportObj.checkinQuestions = window.checkinQuestions || [];
                        exportObj.exportModules.push('checkinQuestions');
                    }
                    if (inclMood) {
                       exportObj.moodLibrary = window.moodLibrary || [];
                       exportObj.exportModules.push('moodLibrary');
                     }
        ...
        function importChatHistory(file) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const hasCheckin = importedData.checkinQuestions && Array.isArray(importedData.checkinQuestions);
                    const hasMood    = importedData.moodLibrary && Array.isArray(importedData.moodLibrary);

                    if (!hasMessages && !hasSettings && !hasReplies && !hasEnvelope && !hasThemes && !hasCheckin && !hasMood) {
                    ...
                    overlay.innerHTML = `
                        <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:88%;max-width:360px;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
                            ...
                            <div style="display:flex;flex-direction:column;gap:9px;margin-bottom:20px;">
                                ...
                                ${makeRow('_imp_checkin', 'fas fa-question-circle', '查岗问题库', '', hasCheckin, false)}
                                ${makeRow('_imp_mood', 'fas fa-smile', '心情库', '', hasMood, false)}
                            </div>
                            <div style="display:flex;gap:10px;">
                    ...
                    if (_impConfirmBtn) _impConfirmBtn.onclick = function() {
                        ...
                        const doCheckin = hasCheckin && !!document.getElementById('_imp_checkin')?.checked;
                        const doMood    = hasMood && !!document.getElementById('_imp_mood')?.checked;

                        if (!doMsgs && !doSettings && !doReplies && !doEnvelope && !doThemes && !doCheckin && !doMood) {showNotification('请至少选择一项导入内容', 'error');return;
                        }
                        ...
                        if (doCheckin && importedData.checkinQuestions) {window.checkinQuestions = importedData.checkinQuestions;
                        }
                        if (doMood && importedData.moodLibrary) {window.moodLibrary = importedData.moodLibrary;
                        }
