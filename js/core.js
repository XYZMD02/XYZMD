...
if (savedMoodLibrary) window.moodLibrary = savedMoodLibrary;
else if (typeof window.moodLibrary === 'undefined') window.moodLibrary = [];
    ...
    try {
        ...
        if (backup && Array.isArray(backup.messages) && backup.messages.length > 0) {
            ...
            if (backup.settings) Object.assign(settings, backup.settings);
            showNotification('已从紧急备份恢复数据', 'warning', 4000);
        } else {
            // 真的没有任何数据时，才使用默认设置，但保留已有数据
            const defaultSettings = getDefaultSettings();
            settings = { ...settings, ...defaultSettings };
        }
    } catch (recoverErr) {
        console.warn('恢复备份也失败了:', recoverErr);
        // 最后保底：保留现有数据，只补全缺失的默认值
        const defaultSettings = getDefaultSettings();
        settings = { ...settings, ...defaultSettings };
    }
    updateUI();
