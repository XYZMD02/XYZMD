...
function manageAutoSendTimer() {
    ...
    if (window.sessionList && window.SESSION_ID) {
        ...
        // 去掉 "session.messages.length > 0" 的限制，允许空会话也能触发
        if (session && !session.paused) {
            // 使用 settings.autoSendInterval（单位：分钟）转换为毫秒
            const intervalMinutes = Math.max(1, settings.autoSendInterval || 5);
            const minDelay = intervalMinutes * 60 * 1000;
            const maxDelay = minDelay * 1.5; // 加点随机范围，更自然
