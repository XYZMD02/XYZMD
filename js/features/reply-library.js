...
function _getGroupCtx(tab) {
    ...
    if (tab === 'checkinQuestions') {
        if (!window.checkinQuestionGroups) window.checkinQuestionGroups = [];
        return { groups: window.checkinQuestionGroups, items: window.checkinQuestions || [], itemLabel: '查岗问题' };
    }
    if (tab === 'phoneStatus') {
        return { groups: window.phoneStatusGroups || [], items: window.phoneStatusLibrary || [], itemLabel: '手机状态' };
    }
    if (tab === 'mood') {
        if (!window.moodGroups) window.moodGroups = [];
        return { groups: window.moodGroups, items: window.moodLibrary || [], itemLabel: '心情' };
    }
...
function _tabHasGroups(tab) {
    tab = tab || currentSubTab;
    return tab === 'custom' || tab === 'pokes' || tab === 'statuses' || tab === 'kaomojis' || tab === 'stickers' || tab === 'moyu' || tab === 'moyuLocations' || tab === 'checkinQuestions' || tab === 'mood';
}
...
function _renderListContentOnly() {
    ...
    if (currentMajorTab === 'reply') {
        ...
        } else if (currentSubTab === 'checkinQuestions') {
            itemsToRender = window.checkinQuestions || [];
        } else if (currentSubTab === 'mood') {
            itemsToRender = window.moodLibrary || [];
        }
...
function _renderModernToolbar() {
    ...
    const isCheckinQTab = currentMajorTab === 'moyu' && currentSubTab === 'checkinQuestions';
    const isMoodTab = currentMajorTab === 'moyu' && currentSubTab === 'mood';
    const hasGroupSupport = _tabHasGroups();
    const canBatch = isMainCustom || isStickersTab || isKaomojisTab || isMoyuTab || isMoyuLocTab || isCheckinQTab || isMoodTab;
...
function deleteItem(index) {
    ...
    else if (currentMajorTab === 'moyu' && currentSubTab === 'checkinQuestions') {
        if (window.checkinQuestions) window.checkinQuestions.splice(index, 1);
    }
    else if (currentMajorTab === 'moyu' && currentSubTab === 'mood') {
        if (window.moodLibrary) window.moodLibrary.splice(index, 1);
    }
    if (item && ctx.groups) {
...
function editItem(index, oldText) {
    ...
    if (currentMajorTab === 'reply' && currentSubTab === 'custom') customReplies[index] = newText.trim();
        ...
        if (window.checkinQuestions) window.checkinQuestions[index] = newText.trim();
    }
    else if (currentMajorTab === 'moyu' && currentSubTab === 'mood') {
        if (window.moodLibrary) window.moodLibrary[index] = newText.trim();
    }
    throttledSaveData();
    renderReplyLibrary();
}
    ...
    panel.querySelector('#ba-confirm').onclick = () => {
        ...
        lines.forEach(val => {
            ...
            const isDup = currentSubTab === 'custom'
                ...
                : currentMajorTab === 'moyu' && currentSubTab === 'checkinQuestions'
                ? (window.checkinQuestions || []).some(r => normalizeStringStrict(r) === norm)
                : currentMajorTab === 'moyu' && currentSubTab === 'mood'
                ? (window.moodLibrary || []).some(r => normalizeStringStrict(r) === norm)
                : false;
            ...
            if (currentSubTab === 'custom') { customReplies.push(val); newItems.push(val); }
                ...
                window.checkinQuestions.push(val); newItems.push(val);
            }
            else if (currentMajorTab === 'moyu' && currentSubTab === 'mood') {
                if (!window.moodLibrary) window.moodLibrary = [];
                window.moodLibrary.push(val); newItems.push(val);
            }
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (currentSubTab === 'kaomojis' ||
                ...
                (currentMajorTab === 'moyu' && (currentSubTab === 'moyu' || currentSubTab === 'moyuLocations' || currentSubTab === 'checkinQuestions' || currentSubTab === 'mood'))) {
            ...
            if (input?.trim()) {
                ...
                else if (currentMajorTab === 'moyu' && currentSubTab === 'checkinQuestions' && (window.checkinQuestions || []).some(r => normalizeStringStrict(r) === valNorm)) isDup = true;
                else if (currentMajorTab === 'moyu' && currentSubTab === 'mood' && (window.moodLibrary || []).some(r => normalizeStringStrict(r) === valNorm)) isDup = true;
                ...
                if (currentSubTab === 'pokes') customPokes.unshift(val);
                    ...
                    if (!window.checkinQuestions) window.checkinQuestions = [];
                    window.checkinQuestions.unshift(val);
                }
                else if (currentMajorTab === 'moyu' && currentSubTab === 'mood') {
                    if (!window.moodLibrary) window.moodLibrary = [];
                    window.moodLibrary.unshift(val);
                }
                throttledSaveData(); renderReplyLibrary();
...
function getCategoryName(tabId) {
    ...
    if (currentMajorTab === 'moyu' && currentSubTab === 'checkinQuestions') return '查岗问题';
    if (currentMajorTab === 'moyu' && currentSubTab === 'phoneStatus') return '手机状态';
    if (currentMajorTab === 'moyu' && currentSubTab === 'mood') return '心情';
