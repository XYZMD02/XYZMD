...
const loadData = async () => {
    try {
        ...
        const results = await Promise.allSettled([
            ...
            localforage.getItem(getStorageKey('moodLibrary')),
        ]);
        ...
        const savedMoodLibrary = getVal(34);
        ...
        if (savedMoodLibrary && Array.isArray(savedMoodLibrary)) window.moodLibrary = savedMoodLibrary;
        ...
        setTimeout(() => {
            ...
            if (typeof startAutoSendLoop === 'function') startAutoSendLoop();
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'moodLibrary',             val: () => localforage.setItem(getStorageKey('moodLibrary'), window.moodLibrary || []) },
...
// ===== 防抖保存函数（500ms 内只执行最后一次）=====
const throttledSaveData = (function() {
    let timer = null;
    return function() {
...
// ===== 主动发送循环（对方定时主动发消息）=====
let autoSendLoopTimer = null;

function startAutoSendLoop() {
    if (autoSendLoopTimer) {
        clearInterval(autoSendLoopTimer);
        autoSendLoopTimer = null;
    }
    if (!settings.autoSendEnabled) return;
    const pool = (customReplies || []).filter(r => r && r.trim());
    if (pool.length === 0) {
        console.warn('[主动发送] 回复库为空，无法启动');
        return;
    }
    const intervalMs = Math.max(1, (settings.autoSendInterval || 5)) * 60 * 1000;

    autoSendLoopTimer = setInterval(() => {
        if (window._pendingReplyTimer) return;
...
window.openMoyuFromNotification = function () {
    ...
    // 根据 autoSendEnabled 启停主动发送
    if (settings.autoSendEnabled) {
        if (typeof startAutoSendLoop === 'function') startAutoSendLoop();
    } else {
        if (typeof stopAutoSendLoop === 'function') stopAutoSendLoop();
    }
};
